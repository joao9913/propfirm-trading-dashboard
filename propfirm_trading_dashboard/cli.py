...
def run_joined_simulation(base_path: str, phase_list: list):
    ...
    runs_table_df = (pd.concat(all_runs, ignore_index=True) if all_runs else pd.Dataframe())
    
    calculator = mc(df_dict)
    all_metrics = calculator.calculate_metrics()
    render_report(all_metrics, "report_html.html", "JoinedSimulation", runs_table=runs_table_df)
